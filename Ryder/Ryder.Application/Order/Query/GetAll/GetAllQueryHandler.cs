    ...
    public class GetAllQueryHandler : IRequestHandler<GetAllQuery, IResult<List<GetAllQueryResponse>>>
    {
        ...
        public async Task<IResult<List<GetAllQueryResponse>>> Handle(GetAllQuery request, CancellationToken cancellationToken)
        {
            ...
            var orderResponses = orders.Select(order => new GetAllQueryResponse
            {
                ...
                Phone = order.PickUpPhoneNumber,
                DropLongitude = order.DropOffLocation.Longitude,
                DropLatitude = order.DropOffLocation.Latitude,
                PickLatitude = order.PickUpLocation.Latitude,
                PickLongitude = order.PickUpLocation.Longitude,
                Amount = order.Amount,
                Status = order.Status,
                Email = order.Email,
                Name = order.Name
